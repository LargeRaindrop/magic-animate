...
class AnimationPipeline_CLIP(DiffusionPipeline):
    _optional_components = []
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(source_image, str):
            ref_image = np.array(Image.open(source_image).convert("RGB").resize((width, height)))[None, :]
        elif isinstance(source_image, np.ndarray):
            ref_image = source_image[None, :]
        ...
        if do_classifier_free_guidance:
            ...
            encoder_hidden_states = encoder_hidden_states.to(device, dtype=text_embeddings.dtype)
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank!=0)):
            ...
            appearance_encoder(
                ref_image_latents.repeat(context_batch_size * (2 if do_classifier_free_guidance else 1), 1, 1, 1),
                t,
                # TODO : use reference_control_reader instead of text_embeddings
                encoder_hidden_states=text_embeddings,
